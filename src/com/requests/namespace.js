 ...
 */
const roots = function (endpoint, id) {
  // Configure the request
  const options = {
    url: `${Helpers.formatEndpoint(endpoint)}/namespace/root/page`,
    method: 'GET',
    headers: Headers.urlEncoded,
    qs: undefined === id ? { pageSize: 100 } : { id, pageSize: 100 },
  };
  // Send the request
  return Send(options);
};
 ...
 */
const mosaicDefinitions = function (endpoint, id) {
  // Configure the request
  const options = {
 ...
 */
const info = function (endpoint, id) {
  // Configure the request
  const options = {
