...
let toMobileKey = function (password, privateKey) {
  ...
  if (!Helpers.isPrivateKeyValid(privateKey))
...
let checkAddress = function (priv, network, _expectedAddress) {
  // Errors
  if (!priv || !network || !_expectedAddress)
    throw new Error("Missing argument !");
  if (!Helpers.isPrivateKeyValid(priv))
    throw new Error("Private key is not valid !");
  //Processing
...
let _encode = function (senderPriv, recipientPub, msg, iv, salt) {
  ...
  if (!senderPriv || !recipientPub || !msg || !iv || !salt)
    throw new Error("Missing argument !");
  if (!Helpers.isPrivateKeyValid(senderPriv))
    throw new Error("Private key is not valid !");
  if (!Helpers.isPublicKeyValid(recipientPub))
  ...
  let encrypted = CryptoJS.AES.encrypt(
    CryptoJS.enc.Hex.parse(convert.utf8ToHex(msg)),
    encKey,
    encIv
  );
  // Result
...
let decode = function (recipientPrivate, senderPublic, _payload) {
  ...
  if (!recipientPrivate || !senderPublic || !_payload)
    throw new Error("Missing argument !");
  if (!Helpers.isPrivateKeyValid(recipientPrivate))
    throw new Error("Private key is not valid !");
  if (!Helpers.isPublicKeyValid(senderPublic))
    throw new Error("Public key is not valid !");
  // Processing
